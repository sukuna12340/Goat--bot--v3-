...
(function ($) {
	...
	HighlightWithinTextarea.prototype = {
		init: function ($el, config) {
			...
			if (this.getType(config) === 'custom') {
				...
				console.error('MASTERBOT: Configuration valide non fournie pour hwt.js');
			}
		},
		...
		fixFirefox: function () {
			...
			let border = this.$highlights.css(['border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width']);
		...
		fixIE: function () {
			...
			this.$highlights.css({ 'word-break': 'break-all' });
		},
				...
				if (plugin) {
					switch (options) {
						...
						case 'destroy':
							...
							break;
						default:
							console.error('MASTERBOT: Méthode "' + options + '" non reconnue');
					}
				} else {
					console.error('MASTERBOT: Plugin doit être initialisé d\'abord');
				}
			} else {
	...
	// Initialisation automatique pour les éléments avec data-hwt
...
console.log('✅ MASTERBOT: hwt.js chargé avec succès');
